...
function DashboardScreenInner() {
  ...
  const calculateRebirthScore = () => {
    ...
    const streakScore = Math.min((totalStreakDays / 30) * 25, 25);
    const supplementScore = Math.min((todaysSupplements / 5) * 25, 25);
    const activeGoals = goals.filter(g => !g.completed).length;
    const goalScore = Math.min((activeGoals / 4) * 25, 25);
